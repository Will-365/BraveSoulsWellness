...
function injectForm(filePath, programName) {
...
`;
    ...
    if (!content.includes('/* ── APPLY FORM CSS ── */')) {
...
<section class="apply" id="apply">
  <div class="container">
    ...
    <div class="apply-form reveal">
      <div class="apply-form-title">Apply for ${programName}</div>
      ...
      <div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:8px">
        ...
        <a href="https://wa.me/250788675638?text=Hi!%20I%20want%20to%20apply%20for%20${encodeURIComponent(programName)}"
...
`;
    ...
    if (!content.includes('<!-- ═══════ APPLY ═══════ -->')) {
        content = content.replace('<footer class="footer">', htmlToInject + '\n<footer class="footer">');
    }
...
`;
    ...
    if (!content.includes('function handleApply(btn)')) {
        content = content.replace('</script>', jsToInject + '\n</script>');
    }
...
injectForm('personaltraining.html', 'Personal Coaching');
injectForm('executivecoaching.html', 'Executive Coaching');
injectForm('communitytraining.html', 'Community Programs');
